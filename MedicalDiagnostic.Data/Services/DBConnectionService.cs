...
namespace MedicalDiagnostic.Data.Services
{
            ...
            Dictionary<string, object>? actionDetails = null)
        {
            ...
            catch (Exception ex)
            {
                _logger.LogWarning(ex,
                    "Audit write failed for action={Action} caseId={CaseId} — clinical flow continues.",
                    action, caseId);
            }
        }
            ...
            Guid? caseId = null)
        {
            try
            {
                ...
                _logger.LogInformation(
                    "✓ Created diagnosis case {CaseId} for patient {PatientId}",
                    diagnosisCase.CaseId, patientId);
...
public async Task UpdateCaseStatusAsync(Guid caseId, string status)
{
    try
    {
        ...
        _logger.LogInformation("✓ Case {CaseId} status → {Status}", caseId, status);
...
public async Task UpdateCasePriorityAsync(Guid caseId, string priority)
{
    try
    {
        ...
        {
            _logger.LogWarning("Case {CaseId} not found when updating priority", caseId);
            return;
        }

        diagnosisCase.Priority = priority;
        diagnosisCase.UpdatedAt = DateTime.UtcNow;

        await _caseRepo.UpdateAsync(diagnosisCase);

        _logger.LogInformation("✓ Case {CaseId} priority → {Priority}", caseId, priority);
    }
    catch (Exception ex)
    {
        _logger.LogError(ex, "✗ Failed to update priority for case {CaseId}", caseId);
        throw;
    }
}
            ...
            Dictionary<string, bool>? familyHistory = null)
        {
            try
            {
                var clinicalSymptom = new ClinicalSymptom
                {
                    ...
                    FamilyHistory = familyHistory != null ? JsonSerializer.Serialize(familyHistory) : null,
                };

                await _symptomRepo.CreateAsync(clinicalSymptom);

                _logger.LogInformation(
                    "✓ Saved symptoms {SymptomId} for case {CaseId}",
        ...
        public async Task<int> SaveImagingResultAsync(
            ...
            byte[]? gradCamOverlay = null,
            byte[]? gradCamHeatmap = null)
        {
            try
            {
                var result = new ImagingResult
                {
                   ...
                   // GradCamOverlayImage = gradCamOverlay,
                    //GradCamHeatmapImage = gradCamHeatmap,
                    //GradCamImageGeneratedAt = (gradCamOverlay != null || gradCamHeatmap != null) 
                      //  ? DateTime.UtcNow 
                        //: null
                };
                ...
                if (gradCamOverlay != null)
                {
                    _logger.LogInformation(
                        " Imaging result id={Id} for case {CaseId}: {Prediction} ({Confidence:P2}) with Grad-CAM ({OverlaySize} bytes)",
                        result.Id, caseId, prediction, confidence, gradCamOverlay.Length);
                }
                else
                {
                    _logger.LogInformation(
                        " Imaging result id={Id} for case {CaseId}: {Prediction} ({Confidence:P2})",
                ...
                await SafeLogAuditAsync(
                    ...
                    actionDetails: new Dictionary<string, object>
                    {
                        { "prediction", prediction },
                        { "confidence", confidence },
                        { "has_gradcam", gradCamOverlay != null }
                    }
                );
            ...
            string? errorMessage = null)
        {
            try
            {
                ...
                _logger.LogInformation(
                    " Clinical result id={Id} for case {CaseId}: {Prediction} ({Confidence:P2})",
                    result.Id, caseId, prediction, confidence);
            ...
            string? errorMessage = null)
        {
            try
            {
                ...
                _logger.LogInformation(
                    "✓ Laboratory result id={Id} for case {CaseId}: {Prediction} ({Confidence:P2})",
                    result.Id, caseId, prediction, confidence);
            ...
            string? errorDetails = null)
        {
            try
            {
                var result = new UnifiedDiagnosisResult
                {
                    ...
                    ExplainabilitySummary = explainabilitySummary != null ? JsonSerializer.Serialize(explainabilitySummary) : null,
                ...
                _logger.LogInformation(
                    " Unified result id={Id} for case {CaseId}: {Diagnosis} ({Confidence:P2})",
        ...
        public async Task<Guid> GetOrCreatePatientAsync(
            ...
            string? gender = null)
        {
            var patient = await _patientRepo.GetByPatientCodeAsync(patientCode);
            ...
            _logger.LogInformation("✓ Created patient {PatientId} code={PatientCode}",
