...
INPUT_TOPIC  = 'brain-imaging-requests'
OUTPUT_TOPIC = 'brain-imaging-results'

MODEL_PATH = os.path.join(os.path.dirname(__file__), 'neurological_resnet18_best.pth')
...
class ImagingService:
    ...
    def predict_with_gradcam(self, pil_image: Image.Image, case_id: str, s3_key: str) -> dict:
        ...
        # Grad-CAM heatmap
        heatmap = self.gradcam.generate_heatmap(image_tensor, target_class=predicted_class)
    ...
    # ------------------------------------------------------------------ #
    def start(self):
        logger.info("Starting Brain Tumor Imaging Service with Grad-CAM...")
        if not self.load_model() or not self.setup_kafka():
            return
        logger.info("Service ready! Listening for imaging requests...")
        try:
    ...
    def cleanup(self):
        ...
        if self.producer:
            self.producer.close()
        logger.info("Brain imaging service stopped")


if __name__ == '__main__':
    ImagingService().start()